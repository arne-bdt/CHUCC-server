...
public class GraphStoreController {
  ...
  public ResponseEntity<?> postGraph(
    ...
    // Handle command
    org.chucc.vcserver.event.VersionControlEvent event = postGraphCommandHandler.handle(command);

    // Handle no-op (null event means no changes - all triples already present)
    if (event == null) {
      return ResponseEntity.status(HttpStatus.NO_CONTENT).build();
    }
